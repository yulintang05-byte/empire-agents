...
ARCHIVE_ROOT_FILES = {
    'QueryEngine.ts': 'QueryEngine.py', 'Task.ts': 'task.py', 'Tool.ts': 'Tool.py',
    'commands.ts': 'commands.py', 'context.ts': 'context.py', 'cost-tracker.ts': 'cost_tracker.py',
    'costHook.ts': 'costHook.py', 'dialogLaunchers.tsx': 'dialogLaunchers.py',
    'history.ts': 'history.py', 'ink.ts': 'ink.py',
    'interactiveHelpers.tsx': 'interactiveHelpers.py', 'main.tsx': 'main.py',
    'projectOnboardingState.ts': 'projectOnboardingState.py', 'query.ts': 'query.py',
    'replLauncher.tsx': 'replLauncher.py', 'setup.ts': 'setup.py',
    'tasks.ts': 'tasks.py', 'tools.ts': 'tools.py',
}

ARCHIVE_DIR_MAPPINGS = {
    'assistant': 'assistant', 'bootstrap': 'bootstrap', 'bridge': 'bridge',
    'buddy': 'buddy', 'cli': 'cli', 'commands': 'commands.py',
    'components': 'components', 'constants': 'constants', 'context': 'context.py',
    'coordinator': 'coordinator', 'entrypoints': 'entrypoints', 'hooks': 'hooks',
    'ink': 'ink.py', 'keybindings': 'keybindings', 'memdir': 'memdir',
    'migrations': 'migrations', 'moreright': 'moreright', 'native-ts': 'native_ts',
    'outputStyles': 'outputStyles', 'plugins': 'plugins', 'query': 'query.py',
    'remote': 'remote', 'schemas': 'schemas', 'screens': 'screens',
    'server': 'server', 'services': 'services', 'skills': 'skills',
    'state': 'state', 'tasks': 'tasks.py', 'tools': 'tools.py',
    'types': 'types', 'upstreamproxy': 'upstreamproxy', 'utils': 'utils',
    'vim': 'vim', 'voice': 'voice',
}
...
class ParityAuditResult:
    ...
    def to_markdown(self) -> str:
        ...
        lines.extend(f'- {item}' for item in self.missing_root_targets) if self.missing_root_targets else lines.append('- none')
        lines.extend(['', 'Missing directory targets:'])
        lines.extend(f'- {item}' for item in self.missing_directory_targets) if self.missing_directory_targets else lines.append('- none')
        return '\n'.join(lines)
...
def run_parity_audit() -> ParityAuditResult:
    ...
    missing_roots = tuple(t for t in ARCHIVE_ROOT_FILES.values() if t not in current_entries)
    missing_dirs = tuple(t for t in ARCHIVE_DIR_MAPPINGS.values() if t not in current_entries)
    current_python_files = sum(1 for p in CURRENT_ROOT.rglob('*.py') if p.is_file())
