...
class QueryEnginePort:
    ...
    def submit_message(self, prompt: str, matched_commands: tuple[str, ...] = (),
        ...
        output = _format_output_fast(prompt, matched_commands, matched_tools, denied_tools,
                                      self.session_id, self.manifest.total_python_files)
    ...
    # [OPT-7] Batch message submission
    def submit_batch(self, prompts: list[str], matched_commands: tuple[str, ...] = (),
                     matched_tools: tuple[str, ...] = ()) -> list[TurnResult]:
        results = []
...
# [OPT-6] String builder pattern: collect parts in a list, join once
def _format_output_fast(prompt, commands, tools, denials, session_id, total_files):
