...
def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(description='Optimized Python porting workspace (v2) for the Claude Code rewrite effort')
    ...
    subparsers.add_parser('parity-audit', help='compare against the local TypeScript archive')
    ...
    list_parser = subparsers.add_parser('subsystems', help='list current Python modules')
    list_parser.add_argument('--limit', type=int, default=32)
    commands_parser = subparsers.add_parser('commands', help='list mirrored command entries')
    ...
    tools_parser = subparsers.add_parser('tools', help='list mirrored tool entries')
    ...
    route_parser = subparsers.add_parser('route', help='route a prompt across inventories')
    route_parser.add_argument('prompt')
    route_parser.add_argument('--limit', type=int, default=5)
    bootstrap_parser = subparsers.add_parser('bootstrap', help='build a runtime session report')
    ...
    flush_parser = subparsers.add_parser('flush-transcript', help='persist and flush a transcript')
    ...
    remote_parser = subparsers.add_parser('remote-mode', help='simulate remote-control branching')
    remote_parser.add_argument('target')
    ssh_parser = subparsers.add_parser('ssh-mode', help='simulate SSH branching')
    ssh_parser.add_argument('target')
    teleport_parser = subparsers.add_parser('teleport-mode', help='simulate teleport branching')
    teleport_parser.add_argument('target')
    direct_parser = subparsers.add_parser('direct-connect-mode', help='simulate direct-connect branching')
    direct_parser.add_argument('target')
    deep_link_parser = subparsers.add_parser('deep-link-mode', help='simulate deep-link branching')
    ...
    exec_command_parser = subparsers.add_parser('exec-command', help='execute a command shim')
    exec_command_parser.add_argument('name')
    exec_command_parser.add_argument('prompt')
    exec_tool_parser = subparsers.add_parser('exec-tool', help='execute a tool shim')
...
def main(argv: list[str] | None = None) -> int:
    ...
    if args.command == 'commands':
        if args.query:
            ...
            output_lines.extend(f'- {m.name} — {m.source_hint}' for m in commands[:args.limit])
            print('\n'.join(output_lines))
        return 0
    if args.command == 'tools':
        if args.query:
            ...
            output_lines.extend(f'- {m.name} — {m.source_hint}' for m in tools[:args.limit])
            print('\n'.join(output_lines))
        return 0
